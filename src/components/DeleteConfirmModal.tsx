...
export const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({
  ...
  if (target.type === 'single') {
    ...
    title = `선택한 ${itemCount}건의 지출 삭제`;
    description = `선택하신 ${itemCount}개의 지출 내역을 모두 삭제하시겠습니까?`;
  } else if (target.type === 'all') {
    itemCount = target.count;
    totalAmount = target.totalAmount;
    title = '전체 지출 내역 일괄 삭제';
    description = `등록된 모든 지출 내역 (${itemCount}건, 총 ${formatCurrency(totalAmount)})을 완전히 삭제하시겠습니까?`;
  }
      ...
      >
        ...
        <div className="flex items-start justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">삭제 작업은 되돌릴 수 없습니다</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="py-4 space-y-3">
          ...
          {target.type === 'all' && (
            <div className="bg-rose-50 border border-rose-200 p-3 rounded-xl text-xs text-rose-800 flex items-start gap-2">
              ...
              <span>
                <strong>주의:</strong> 모든 지출 내역이 영구적으로 지워지며 카테고리 집계 통계도 함께 초기화됩니다.
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-bold text-white rounded-xl shadow-xs flex items-center gap-1.5 transition-colors ${
              target.type === 'all'
          ...
          >
            ...
            <span>{target.type === 'all' ? '전체 삭제하기' : '삭제 확인'}</span>
          </button>
        </div>
      </div>
    </div>
