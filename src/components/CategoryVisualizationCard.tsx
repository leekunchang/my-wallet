...
import { ExpenseItem, CategorySummary, CATEGORIES, CycleInfo } from '../types';
...
export const CategoryVisualizationCard: React.FC<CategoryVisualizationCardProps> = ({
  ...
  if (targetExpenses.length === 0) {
    return (
      <div className="bg-white rounded-2xl p-6 border border-slate-200/90 shadow-sm text-center">
        ...
        <h3 className="text-base font-bold text-slate-800">카테고리별 소비 분석</h3>
        <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
          가계부에서 지출 내역을 등록하면 AI가 분류한 카테고리별 지출 표와 시각화 차트가 이곳에 표시됩니다.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/90 shadow-sm space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold text-slate-900 tracking-tight">카테고리별 지출 시각화 표 및 통계</h3>
              <span className="text-xs font-semibold px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-100">
                {summaries.length}개 카테고리
              </span>
            </div>
            <p className="text-xs text-slate-500">
              {isAllFallback
                ? '등록된 전체 지출 기준 카테고리 분석'
                : `이번 주기 (${cycleInfo.paydayDay}일 기준) 총 ${formatCurrency(totalAmount)} 소비 분석`}
            </p>
          </div>
        </div>
        ...
        {topCategory && (
          <div className="flex items-center gap-2 bg-indigo-50/80 px-3 py-1.5 rounded-xl border border-indigo-100 self-start sm:self-auto text-xs">
            ...
            <span className="text-slate-600 font-medium">최다 지출:</span>
      ...
      {/* 2. 상단 다중 세그먼트 비중 시각화 바 (Stacked Progress Bar) */}
      <div>
        <div className="flex items-center justify-between text-xs font-semibold text-slate-600 mb-2">
          ...
          <span className="text-slate-400">총 {totalCount}건 지출</span>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between text-xs font-bold text-slate-800 mb-2">
          ...
          <span className="text-slate-400 font-normal">금액 높은 순 정렬</span>
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200">
              <tr>
                ...
                <th scope="col" className="py-2.5 px-3 text-center w-16">건수</th>
                <th scope="col" className="py-2.5 px-3 text-right w-20">점유율</th>
                <th scope="col" className="py-2.5 px-3 w-36">비중 시각화</th>
              </tr>
            </thead>
            ...
            <tfoot className="bg-slate-50 font-bold border-t border-slate-200 text-slate-900">
              <tr>
                ...
                <td className="py-2.5 px-3">{summaries.length}개 분야</td>
        ...
        <div className="bg-slate-50 rounded-xl p-4 border border-indigo-100 text-xs">
          <div className="flex items-center justify-between mb-2">
            <h4 className="font-bold text-slate-900 flex items-center gap-1.5">
              ...
              <span>{CATEGORIES.find((c) => c.key === selectedCategoryKey)?.name} 지출 내역만 보기</span>
            </h4>
            <button
