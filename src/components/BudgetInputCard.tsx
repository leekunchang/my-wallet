    ...
    <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/90 shadow-sm transition-all">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-bold text-slate-800 tracking-tight">생활비 금액 입력</h2>
        </div>
        <span className="text-xs text-slate-400">실시간 자동 계산 및 저장</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-slate-50/70 rounded-xl p-4 border border-slate-200/80">
          ...
          <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
            <span className="text-[11px] text-slate-400 mr-1">자주 쓰는 금액:</span>
        ...
        <div className="bg-slate-50/70 rounded-xl p-4 border border-slate-200/80">
          ...
          {/* 잔액 미세 조정 버튼 */}
          <div className="flex flex-wrap items-center gap-1.5 mt-2.5">
            <span className="text-[11px] text-slate-400 mr-1">간편 증감:</span>
            <button
              onClick={() => adjustBalance(100000)}
              className="text-xs px-2 py-1 rounded-md font-medium bg-white text-slate-600 border border-slate-200 hover:bg-slate-100 flex items-center gap-0.5"
            >
            ...
            {depositAmount > 0 && currentBalance !== depositAmount && (
              <button
                ...
                title="잔액을 입금액 전액으로 리셋"
                className="text-xs px-2 py-1 rounded-md font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 flex items-center gap-0.5 ml-auto"
              >
                <RotateCcw className="w-3 h-3" /> 전액
              </button>
            )}
          </div>
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-3">
          <span className="text-slate-500">
            지금까지 사용한 금액: <strong className="text-slate-800 font-semibold">{formatCurrency(usedAmount)}</strong>
          </span>
        </div>
